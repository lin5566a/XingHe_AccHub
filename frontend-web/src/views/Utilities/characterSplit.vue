        ...
        <div class="flex py-3-5 gap-3.5 items-center">
          <div class="flex items-center flex-col sm:flex-row gap-3.5 sm:flex-1 w-full">
            <div class="flex items-center flex-col sm:flex-row sm:flex-1 w-full gap-3.5 ">                    
              <el-input type="text" class="input-text flex-grow" placeholder="请输入换行字符" v-model="splitData"/>
            </div>
            <div class="flex items-center flex-row  gap-3.5">
              <el-button type="primary" class="btn-split bg-from-blue-600 px-2 sm:px-4 text-sm sm:text-base rounded-md " @click="handleExecute"> 执行</el-button>
              <el-button type="primary" class="btn-split bg-from-blue-600 px-2 sm:px-4 text-sm sm:text-base  rounded-md " @click="handleReset">重置</el-button>
              
              <el-button type="primary" class="btn-split bg-from-blue-600 icon-btn rounded-md" @click="handleDownload" v-if='outputData'>
...
<script setup>
  ...
  const handleExecute = () => {   
    if (!inputData.value || !splitData.value) return
    outputData.value = inputData.value.split(splitData.value).join('\n')
  }
